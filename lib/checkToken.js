...
{
	if (!storageRecord.facebookDialogOAuthResponse) {log("Error: facebookOAuthResponse is undefined!"); return false;}
	...
	if (typeof res == "undefined") {log("Error: facebookOAuthResponse URL/content empty!"); return false;}
	if (res.indexOf('access_token')!=-1) {
		log("Access_token exists in this traffic.");
		log("Now try to verify this exploit");
		return true;
	}
	else {
		log("Access_token NOT spotted in this traffic.");
		return false;
	}
}
...
function checkLoginButtonRemoved(){
	...
	ccc.pressLoginButtonWorker().port.on("after_modification_sendLoginButtonInformation",  function(response){
		log("Phase "+(inheritedPhase+5).toString()+": checking login button existence after modification...");
		ccc.setCapturingPhase(ccc.capturingPhase() + 1);
		log("Current login button XPath is: " + response.loginButtonXPath);
...
function checkStructuralDiff(){
	ccc.testSuiteWorker().port.on("after_modification_extractedContent",function(response){
		...
		//log("Phase "+(inheritedPhase+6).toString()+": Saved response content to file.");
		log("Phase "+(inheritedPhase+7).toString()+": Checking extracted content and identifying session owner...");
		...
		log("This website displays " + (displayFirstName ? "first name, ":"" ) + (displayLastName ? "last name, ":"" ) + (displayEmail ? "email, ":"" ) + (displayPicSRC ? "picsrc, ":"" ) + (displayPicSRC2 ? "picsrc2, ":"" ) + (displayPicSRC3 ? "picsrc3, ":"" ) + (displayPicSRC4 ? "picsrc4.":"" ));
		...
		if (sessionAScore > 0 && sessionBScore == 0) log("Web application now logged in as session A, threat successful, sessionAscore is: " + sessionAScore.toString());
		else if (sessionBScore > 0 && sessionAScore == 0) log("Web application now logged in as session B, threat failed, sessionBscore is: " + sessionBScore.toString());
		else log("Cannot determine login state, here are the scores: "+sessionAScore.toString()+ " " + sessionBScore.toString());
	});
...
function processBuffer(url)
{
	...
	if (capturingPhase == inheritedPhase+2 && checkAgainstFilter(url, capturingPhase) && ccc.loginButtonClicked() && sawDialogOAuth)
	{
		...
		log("Phase "+(inheritedPhase+2).toString()+": Saw FB visit, waiting for access_token pattern to appear");
		ccc.setCapturingPhase(capturingPhase + 1);
	}
	if (capturingPhase == inheritedPhase+4 && checkAgainstFilter(url, capturingPhase)) {
		log("Phase "+(inheritedPhase+4).toString()+": revisited the site after access_token is modified, ready to compare credentials/differences.");
...
function delayRefreshTestTab()
{
	...
	if (ccc.capturingPhase() == inheritedPhase + 4) {
		log("Sub-Phase"+(inheritedPhase+3).toString()+".5: Site probably used JS to manipulate cookies, now refresh the site and try to capture cookies");
...
TracingListener.prototype =
{
    onDataAvailable: function(request, context, inputStream, offset, count)
    {
		...
		if (ccc.usedFBSDK() && ccc.capturingPhase() == inheritedPhase + 3)
		{
			if (data.substr(0,42) == '<script type="text/javascript">var message' && data.indexOf('access_token')!=-1){
				log("Phase "+(inheritedPhase+3).toString()+": App using SDK: trying to modify access_token.");
    ...
    onStartRequest: function(request, context) {
		...
		{
			log("Phase "+(inheritedPhase+3).toString()+": App does not use SDK: trying to modify access_token.");
			...
			if (checkAgainstFilter(url, ccc.capturingPhase())){
				if (ccc.capturingPhase() == inheritedPhase + 3 && !ccc.usedFBSDK())
				{
					...
					try {
						...
						log("Redirect Domain changed to: " + redirectDomain);
					}
					catch(ex){};
				}
				if (ccc.capturingPhase() == inheritedPhase + 4)
...
exports.processLoaded = function(url)
{
	...
	if (checkAgainstFilter(url,capturingPhase)){
		log("Phase "+(inheritedPhase+1).toString()+": cleared cookies, revisited the site. Now ready to send exploit request.");
