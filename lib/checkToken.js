...
{
	var i = 0;
	if (capturingPhase == inheritedPhase + 2){
...
function checkLoginButtonRemoved(){
	...
	ccc.pressLoginButtonWorker().port.on("after_modification_sendLoginButtonInformation",  function(response){
		console.log("Phase "+(inheritedPhase+5).toString()+": checking login button existence after modification...");
	...
	ccc.pressLoginButtonWorker().port.emit("after_modification_sendLoginButtonInformation", {"indexToClick": ccc.indexToClick(), "tryFindInvisibleLoginButton":ccc.tryFindInvisibleLoginButton()});
}
...
function checkStructuralDiff(){
	ccc.testSuiteWorker().port.on("after_modification_extractedContent",function(response){
		...
		//console.log("Phase "+(inheritedPhase+6).toString()+": Saved response content to file.");
		...
		ccc.automateSSOWorker().port.on("requestAccountInfo",function (resp){
			console.log("Phase "+(inheritedPhase+7).toString()+": Checking extracted content and identifying session owner...");
			accounts = JSON.parse(resp);
			
			if (ccc.responseTextContent()[1].indexOf(accounts[0].firstName)!=-1&&ccc.responseTextContent()[2].indexOf(accounts[1].firstName)!=-1) displayFirstName = true;
			if (ccc.responseTextContent()[1].indexOf(accounts[0].lastName)!=-1&&ccc.responseTextContent()[2].indexOf(accounts[1].lastName)!=-1) displayLastName = true;
			if (ccc.responseTextContent()[1].indexOf(accounts[0].email)!=-1&&ccc.responseTextContent()[2].indexOf(accounts[1].email)!=-1) displayEmail = true;
			...
			console.log("This website displays " + (displayFirstName ? "first name, ":"" ) + (displayLastName ? "last name, ":"" ) + (displayEmail ? "email, ":"" ) + (displayPicSRC ? "picsrc, ":"" ) + (displayPicSRC2 ? "picsrc2, ":"" ) + (displayPicSRC3 ? "picsrc3, ":"" ) + (displayPicSRC4 ? "picsrc4.":"" ));
			
			var sessionAScore = 0;
			var sessionBScore = 0;
			if (displayFirstName && modifiedResponseContent.indexOf(accounts[0].firstName)!=-1) sessionAScore++;
			if (displayLastName && modifiedResponseContent.indexOf(accounts[0].lastName)!=-1) sessionAScore++;
			if (displayEmail && modifiedResponseContent.indexOf(accounts[0].email)!=-1) sessionAScore++;
			...
			if (displayFirstName && modifiedResponseContent.indexOf(accounts[1].firstName)!=-1) sessionBScore++;
			if (displayLastName && modifiedResponseContent.indexOf(accounts[1].lastName)!=-1) sessionBScore++;
			if (displayEmail && modifiedResponseContent.indexOf(accounts[1].email)!=-1) sessionBScore++;
			...
			if (sessionAScore > 0 && sessionBScore == 0) console.log("Web application now logged in as session A, threat successful");
			else if (sessionBScore > 0 && sessionAScore == 0) console.log("Web application now logged in as session B, threat failed");
			else console.log("Cannot determine login state, here are the scores: "+sessionAScore.toString()+ " " + sessionBScore.toString());
		});
		ccc.automateSSOWorker().port.emit("requestAccountInfo","");
	});
...
function processBuffer(url)
{
	...
	if (capturingPhase == inheritedPhase+2 && checkAgainstFilter(url, capturingPhase) && ccc.loginButtonClicked() && sawDialogOAuth)
	{
		...
		console.log("Phase "+(inheritedPhase+2).toString()+": Saw FB visit, waiting for access_token pattern to appear");
		ccc.setCapturingPhase(capturingPhase + 1);
	}
	if (capturingPhase == inheritedPhase+4 && checkAgainstFilter(url, capturingPhase)) {
		console.log("Phase "+(inheritedPhase+4).toString()+": revisited the site after access_token is modified, ready to compare credentials/differences.");
...
function delayRefreshTestTab()
{
	...
	if (ccc.capturingPhase() == inheritedPhase + 4) {
		console.log("Sub-Phase"+(inheritedPhase+3).toString()+".5: Site probably used JS to manipulate cookies, now refresh the site and try to capture cookies");
...
TracingListener.prototype =
{
    onDataAvailable: function(request, context, inputStream, offset, count)
    {
		...
		if (ccc.usedFBSDK() && ccc.capturingPhase() == inheritedPhase + 3)
		{
			if (data.substr(0,42) == '<script type="text/javascript">var message' && data.indexOf('access_token')!=-1){
				console.log("Phase "+(inheritedPhase+3).toString()+": App using SDK: trying to modify access_token.");
    ...
    onStartRequest: function(request, context) {
		...
		{
			console.log("Phase "+(inheritedPhase+3).toString()+": App does not use SDK: trying to modify access_token.");
...
exports.processLoaded = function(url)
{
	var capturingPhase = ccc.capturingPhase();
	sawDialogOAuth = false;
	if (capturingPhase == inheritedPhase + 1 && url == ccc.siteToTest()){
		console.log("Phase "+(inheritedPhase+1).toString()+": cleared cookies, revisited the site. Now ready to send exploit request.");
