...
{
	var i = 0;
	if (capturingPhase == inheritedPhase + 2 && (url.indexOf("http://www.facebook.com/dialog/return")==0 || url.indexOf("https://www.facebook.com/dialog/return")==0)) 
	{
	...
	else if (capturingPhase == inheritedPhase + 4 && delayRefreshCalled)
	{
...
function checkLoginButtonRemoved(){
	...
	ccc.pressLoginButtonWorker().port.on("after_modification_sendLoginButtonInformation",  function(response){
		...
		if (response.loginButtonXPath == ccc.loginButtonXPath() || response.loginButtonOuterHTML == ccc.loginButtonOuterHTML()) {
			...
			observerService.removeObserver(httpRequestObserver, "http-on-examine-response");
			checkCode.init(inheritedPhase+6);
			return;
		}
...
function checkStructuralDiff(){
	ccc.testSuiteWorker().port.on("after_modification_extractedContent",function(response){
		...
		if (sessionAScore > 0 && sessionBScore == 0) {
			...
			observerService.removeObserver(httpRequestObserver, "http-on-examine-response");
			checkCode.init(inheritedPhase+7);
		}
			...
			ccc.log(ccc.siteToTest() + " cannot be determined (score error in [1]).", true);
			observerService.removeObserver(httpRequestObserver, "http-on-examine-response");
			checkCode.init(inheritedPhase+7);
		}
	});
...
function delayRefreshTestTab()
{
	...
	if (ccc.capturingPhase() == inheritedPhase + 4) {
		ccc.log("Sub-Phase "+(inheritedPhase+3).toString()+".5: revisiting the testing site.");
		try {
...
TracingListener.prototype =
{
    onDataAvailable: function(request, context, inputStream, offset, count)
    {
		...
		if (ccc.usedFBSDK() && ccc.capturingPhase() == inheritedPhase + 3)
		{
			if (data.substr(0,42) == '<script type="text/javascript">var message' && data.indexOf('access_token')!=-1){
				...
				ccc.setCapturingPhase(inheritedPhase+4);
			}
		}
    ...
    onStartRequest: function(request, context) {
		...
		if ((!ccc.usedFBSDK()) && uri.indexOf('access_token')!=-1)
		{
			...
			ccc.log("Original URI is:" + uri);
			var tail = uri.substr(uri.indexOf('access_token'), uri.length);
			tail = tail.substr(tail.indexOf('&'),tail.length);
			request.URI.spec = uri.substr(0,uri.indexOf('access_token'))+"access_token="+ccc.accountsInfo()[0].access_token+tail;							//redirect URI to the threat generated.
			if (delayRefreshTestTabTimer) window.clearTimeout(delayRefreshTestTabTimer);
			delayRefreshTestTabTimer = window.setTimeout(delayRefreshTestTab,15000);
			ccc.setCapturingPhase(inheritedPhase+4);
		}
...
{
	if (ccc.capturingPhase()==inheritedPhase+3) return true;
}
