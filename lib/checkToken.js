...
function checkLoginButtonRemoved(){
	...
	try{
		...
		if (checkLoginButtonRemovedAttempts < 2){
			...
			ccc.log("pressloginworker hidden frame error - tried too many times, give up.");
		}
	}
}
...
function checkStructuralDiff(){
	ccc.testSuiteWorker().port.on("after_modification_extractedContent",function(response){
		...
		if (sessionAScore > 0 && sessionBScore == 0) ccc.log("Web application now logged in as session A, threat successful, sessionAscore is: " + sessionAScore.toString());
		else if (sessionBScore > 0 && sessionAScore == 0) ccc.log("Web application now logged in as session B, threat failed, sessionBscore is: " + sessionBScore.toString());
		else ccc.log("Cannot determine login state, here are the scores: "+sessionAScore.toString()+ " " + sessionBScore.toString());
	});
