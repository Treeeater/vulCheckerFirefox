...
var testNext = function(){
	if (!testSucceed && !firstTimeStart) {
		if (timedOut) {
			if (stalledSites.indexOf(testList[i-1])==-1) {
				stalledSites.push(testList[i-1]);
			...
			ccc.log("Test failed on " + testList[i-1], true);
		}
	}
	if (!firstTimeStart && ccc.supportFBLogin() && !retry) {
		...
		if ((!PT.vulStatus().code_vul) && ((ccc.detectionMode() & CONST.dm.code_vul) != 0)) {
			ccc.log(testList[i-1] + " is not vulnerable to [2].");
			ccc.log(testList[i-1] + " is not vulnerable to [2].", true);
		}
		if ((!PT.vulStatus().referrer_vul) && ((ccc.detectionMode() & CONST.dm.referrer_vul) != 0)) {
			ccc.log(testList[i-1] + " is not vulnerable to [4].");
			ccc.log(testList[i-1] + " is not vulnerable to [4].", true);
		}
		if ((!PT.vulStatus().secret_in_body_vul) && ((ccc.detectionMode() & CONST.dm.secret_in_body_vul) != 0)) {
			ccc.log(testList[i-1] + " is not vulnerable to [5].");
			ccc.log(testList[i-1] + " is not vulnerable to [5].", true);
		}
	...
	if (!retry) {
		var filePath = file.join(profilePath, "testResults", ccc.fileNameSanitize(testList[i]));
		while (file.exists(filePath)){
			...
			filePath = file.join(profilePath, "testResults", ccc.fileNameSanitize(testList[i]));
		}
	}
	testSucceed = false;
	timedOut = true;
