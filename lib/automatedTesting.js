...
var testNext = function(){
	...
	if (!firstTimeStart && ccc.supportFBLogin()) {
		...
		if ((!PT.vulStatus().code_vul) && ((ccc.detectionMode() & CONST.dm.code_vul) != 0)) {
			ccc.log(testList.testList[i-1] + " is not vulnerable to [2].");
			ccc.log(testList.testList[i-1] + " is not vulnerable to [2].", true);
		}
		if ((!PT.vulStatus().referrer_vul) && ((ccc.detectionMode() & CONST.dm.referrer_vul) != 0)) {
			ccc.log(testList.testList[i-1] + " is not vulnerable to [4].");
			ccc.log(testList.testList[i-1] + " is not vulnerable to [4].", true);
		}
		if ((!PT.vulStatus().secret_in_body_vul) && ((ccc.detectionMode() & CONST.dm.secret_in_body_vul) != 0)) {
			ccc.log(testList.testList[i-1] + " is not vulnerable to [5].");
			ccc.log(testList.testList[i-1] + " is not vulnerable to [5].", true);
		}
