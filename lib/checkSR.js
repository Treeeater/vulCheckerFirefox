...
var checkAgainstFilter = function(url, capturingPhase)
{
	...
	else if (!ccc.usedFBSDK() && ccc.redirectDomain() != "" && capturingPhase == inheritedPhase + 2)
	{
		...
		if (redirectDomain[redirectDomain.length-1] == '/') redirectDomain = redirectDomain.substr(0,redirectDomain.length-1);
		if (url.startsWith(redirectDomain) && ccc.credentialsInserted()) {
			return true;
		}
	}
	return false;
...
function checkLoginButtonRemoved(){
	ccc.setCredentialsInserted(false);			//consume it.
	...
	try{
		ccc.pressLoginButtonWorker().port.emit("after_modification_sendLoginButtonInformation", {"indexToClick": ccc.indexToClickLoginButton(), "tryFindInvisibleLoginButton":ccc.tryFindInvisibleLoginButton(), "account":ccc.accountsInfo()});
	} catch(ex){
