...
var nextModule = function (success){
	...
	ccc.inBetweenModule();
	cleanup();
	automatedTesting.finishedTesting(success);
}
...
var checkAgainstFilter = function(url, capturingPhase)
{
	...
	else if (!ccc.usedFBSDK() && ccc.redirectDomain() != "" && capturingPhase == inheritedPhase + 2)
	{
		...
		if (url.startsWith(ccc.redirectDomain()) && ccc.credentialsInserted()) {
			return true;
		}
	}
	return false;
...
function checkStructuralDiff(){
	ccc.testSuiteWorker().port.on("after_modification_extractedContent",function(response){
		...
		if (sessionAScore > 0 && sessionBScore == 0) {
			...
			ccc.log(ccc.siteToTest() + " cannot be determined (score error in [3]).", true);
			nextModule(false);
		}
	});
...
TracingListener.prototype =
{
    onDataAvailable: function(request, context, inputStream, offset, count)
    {
		...
		if (ccc.usedFBSDK() && ccc.capturingPhase() == inheritedPhase + 2)
		{
			...
			if (data.substr(0,42) == '<script type="text/javascript">var message' && data.indexOf('signed_request=')!=-1){
...
var httpRequestObserver =
{
    observe: function(aSubject, aTopic, aData)
    {
        if (aTopic == "http-on-examine-response")
        {
			...
			else if (uri.indexOf('signed_request=')!=-1)
			{
