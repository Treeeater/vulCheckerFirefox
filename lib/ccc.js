...
var SubmitButtonClickDepth = 2;
var LoginButtonCandidateSize = 5;
var SubmitButtonCandidateSize = 2;
var elementsToFill = [];
var buttonToClick = [];
...
function clickLoginButton(){
	...
	assume(capturingPhase <= 5, "assumption click login button failure");
	...
	try {
		if (indexToClickThisAttempt >= sortedCandidates.length) {
			console.log("no (more) login button found on this subpage, fast forwarding.");
			if (searchingForLoginButton){
		...
		console.log("clicking on "+(indexToClickThisAttempt+1).toString()+"th ranked element in the pool:");
...
function tryToRegister(){
	...
	catch(ex){log("pressSubmitButtonWorker hidden frame error 1");}
			console.log('here8');
...
exports.initPressLoginButton = function(worker){
	...
	pressLoginButtonWorker.port.on("reportCandidates", function(response){
		...
		if (capturingPhase == 5 || capturingPhase == 11 || capturingPhase == 4 || capturingPhase == 10){
...
exports.initRegistrationWorker = function(worker){
	...
	registrationWorker.port.on("reportSubmitButtonCandidates", function(response){
		submitButtonPool.push({worker:registrationWorker, result:response});
	});
	if (typeof accountsInfo != "undefined"){
		try {registrationWorker.port.emit("init",{"accountsInfo":accountsInfo[FBAccount-1], "debug":debug});} catch (ex) {}
	}
}
