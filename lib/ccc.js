...
var IdPDomains = ["https://www.facebook.com/dialog/oauth", "https://www.facebook.com/v1.0/dialog/oauth", "https://www.facebook.com/dialog/permissions.request", "https://www.facebook.com/v1.0/dialog/permissions.request", "https://www.facebook.com/login.php", "http://www.facebook.com/dialog/oauth", "http://www.facebook.com/v1.0/dialog/oauth"];
...
{
	var i = 0;
	if ((capturingPhase == 2 || capturingPhase == 8) && (url.indexOf("http://www.facebook.com/dialog/return")==0 || url.indexOf("https://www.facebook.com/dialog/return")==0 || url.indexOf("http://www.facebook.com/v1.0/dialog/return")==0 || url.indexOf("https://www.facebook.com/v1.0/dialog/return")==0)) 
	{
		return true;
	}
...
var deleteCookies = function(){
	...
	try {
		...
		//new firefox api, deprecated earlier cacheService:
...
function startOver(regDone){
	...
	registrationNeeded = (!regDone) && (conf.registrationNeeded || false);
	searchForSignUpForFB = (!regDone) && (conf.searchForSignUpForFB || false);
...
function recordLoginStatus(recordString, futileClick, dupClick){
	...
	for (i = 0; i < loginButtonClickHistory.length; i++){
		...
		returnString += "," + loginButtonClickHistory[i].width;			//indicate this is not the correct element to click.
		returnString += "," + loginButtonClickHistory[i].height;			//indicate this is not the correct element to click.
		returnString += "," + loginButtonClickHistory[i].type;			//indicate this is not the correct element to click.
		returnString += "," + loginButtonClickHistory[i].x;			//indicate this is not the correct element to click.
		returnString += "," + loginButtonClickHistory[i].y;			//indicate this is not the correct element to click.
...
function clickLoginButton(){
	...
	if (loginClickAttempts >= LoginButtonClickDepth) {
		...
		if (!!conf.USENIX.experiments.exhaustiveSearchAndRecord){
	...
	try {
		...
		//log("score: "+sortedCandidates.map(function(ele, index, arr) {return ele.score.toString()}).join(","));
...
function testSuitePhase2(url){
	...
	if (!!conf.USENIX.experiments.recordLoginButton || !!conf.USENIX.experiments.exhaustiveSearchAndRecord){
...
function processBuffer(url)
{
	...
	if (capturingPhase == 2 && checkAgainstFilter(url, capturingPhase) && loginButtonClicked && sawDialogOAuth)
	{
		testSuitePhase2(url);
		sawDialogOAuth = false;
		if (!conf.USENIX.experiments.exhaustiveSearchAndRecord) searchingForLoginButton = false;			//in login button experiments, we never stop searching for login button.
		return;
	}
			...
			if (checkAgainstFilter(url, capturingPhase)){
				...
				if (url.startsWith("https://www.facebook.com/dialog/oauth") || url.startsWith("http://www.facebook.com/dialog/oauth") || url.startsWith("https://www.facebook.com/v1.0/dialog/oauth") || url.startsWith("http://www.facebook.com/v1.0/dialog/oauth")) {
					...
					if (url.indexOf("static.ak.facebook.com")==-1) {
						...
						if (temp.indexOf("http://www.facebook.com/dialog/return")!=0 && temp.indexOf("https://www.facebook.com/dialog/return")!=0 && temp.indexOf("http://www.facebook.com/v1.0/dialog/return")!=0 && temp.indexOf("https://www.facebook.com/v1.0/dialog/return")!=0) {
				...
				if ((url.startsWith("https://www.facebook.com/dialog/oauth") || url.startsWith("http://www.facebook.com/dialog/oauth") || url.startsWith("https://www.facebook.com/v1.0/dialog/oauth") || url.startsWith("http://www.facebook.com/v1.0/dialog/oauth")) && !FBSDKDetermined && notAppsFacebookComDomain && isHostRelatedDomain){
					...
					if (url.indexOf("static.ak.facebook.com")==-1) 
					{
						...
						if (redirectDomain=="")
						{
							...
							if (redirectDomain.indexOf("http://www.facebook.com/dialog/return")==0 || redirectDomain.indexOf("http://www.facebook.com/v1.0/dialog/return")==0 || redirectDomain.indexOf("https://www.facebook.com/dialog/return")==0 || redirectDomain.indexOf("https://www.facebook.com/v1.0/dialog/return")==0) {
								loginButtonClicked = true;			//this must be clicked from an iframe, let's make sure this var is set to true.
								log("Site uses social plugin button.php, redirect domain changed to http://static.ak.facebook.com/connect/xd_arbiter");
								redirectDomain = "http://static.ak.facebook.com/connect/xd_arbiter";			//FB changed as of May 27 2014, now either url (http or https) can be redirect_url, and does not depend on the if conditions here. Therefore we use socialButton boolean to mark things.
								socialButton = true;
							}
							else {
				...
				if ((url.startsWith("https://www.facebook.com/dialog/oauth") || url.startsWith("http://www.facebook.com/dialog/oauth") || url.startsWith("https://www.facebook.com/v1.0/dialog/oauth") || url.startsWith("http://www.facebook.com/v1.0/dialog/oauth")) && notAppsFacebookComDomain && isHostRelatedDomain)
				{
					sawDialogOAuth = true;
				}
...
exports.initAutomateSSOWorker = function(worker){
	...
	worker.port.on("requestFBAccount",function(){
		if (capturingPhase == 3){
			if (!!conf.USENIX.experiments.searchLoginButtonOnly)
			{
				automatedTesting.finishedTesting(true);
				return;
			}
...
exports.initRegistrationWorker = function(worker){
	...
	if (capturingPhase != 4 && capturingPhase != 10 && worker.tab.i != 1) return;		//don't bother if it is not time to register.
