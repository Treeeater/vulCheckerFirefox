...
function testSuitePhase2(url){
	...
	var i = 0;
	for (i = 0; i < loginButtonClickHistory.length; i++){
		stats += (loginButtonClickHistory[i].stats + "+");
		stringSig += (loginButtonClickHistory[i].stringSig + "+");
	}
	log("stats: " + stats,true);
	log("stringSig: " + stringSig, true);
...
exports.initPressLoginButton = function(worker){
	...
	if (capturingPhase == 2 || capturingPhase == 8 || checkToken.shouldClickLoginButton() || checkSR.shouldClickLoginButton()) {
		clickLoginButtonTimer = window.setTimeout(collectCandidatesAndClickNext, 5000);
	}
...
exports.setClickLoginButtonTimer = function(t){
	if (clickLoginButtonTimer) window.clearTimeout(clickLoginButtonTimer);			//must clear this timer, otherwise phase 16 (checkSR) might have problem (executing search twice, the second time will ignore the first element).
	if (checkLoginButtonRemovedTimer) window.clearTimeout(checkLoginButtonRemovedTimer);
	clickLoginButtonTimer = window.setTimeout(collectCandidatesAndClickNext, t);
};
