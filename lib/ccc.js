...
//var old_token = "CAADxRthhGccBANJYTi9ZCZAlAX6PJo1hrs4tnRTe51gpwS4H8SmhxkJofVODeGEY4dsuo4m3LcojXy0H8nQPMao1NCfWQtC1YI6shKvuXfZClhCYmZCZASiRa7tgyFNQuURf4jXWzPbuIJDyYcaYRej1SUAbAjp4ZD";
var old_token = "";
var old_code = "";
...
function tryToRegister(){
	assume(capturingPhase == 4, "tryToRegister violation");
...
function delayRefreshTestTab()
{
	...
	assume(capturingPhase == 4 || capturingPhase == 10, "delayRefreshTestTab violation");
	...
	pressLoginButtonWorker.port.on("sendLoginButtonInformation", function(response){
		...
		if (response.loginButtonXPath == loginButtonXPath && response.loginButtonOuterHTML == loginButtonOuterHTML) {
			...
			if (registerAttempts < 2)
			{
				...
				console.log("This site needs registration, now trying again to finish registration for the " + registerAttempts.toString() + "th time...");
				try {testSuiteWorker.port.emit("action",{"action": "navigateTo", "site":siteToTest});} catch(ex){console.log("testSuiteWorker hidden frame error");}
				return;
			}
			else {
...
exports.initAutomateSSOWorker = function(worker){
	...
	automateSSOWorker.port.on("requestAccountInfo",function(response){
		registrationWorker.port.emit("issueUserInfo",response[FBAccount-1]);
	});
...
exports.initRegistrationWorker = function(worker){
	...
	registrationWorker.port.on("submitted",function(response){
		buttonToClick = response.buttonToClick;
		elementsToFill = response.elementsToFill;
		setTimeout(delayRefreshTestTab,10000);			//after 10 seconds, refresh the homepage.
	});
	registrationWorker.port.on("getUserInfo",function(response){
