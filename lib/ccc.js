...
function collectCandidatesAndClickNext(){
	...
	clickLoginButtonTimer = window.setTimeout(clickLoginButton,3000);
	return;
}

function recordLoginStatus(recordString){
...
function fastForwardNextLoginButton(){
	...
	} catch(ex){log("testSuiteWorker hidden frame error 2");}
}
...
function clickLoginButton(){
		...
		if (indexToClickLoginButton >= Math.pow(LoginButtonCandidateSize,LoginButtonClickDepth)){
			...
			if (searchingForLoginButton){
				...
				if (conf.USENIX.experiments.testLoginButton){
					...
					recordString = recordLoginStatus(recordString);
	...
	if (indexToClickLoginButton >= Math.pow(LoginButtonCandidateSize, LoginButtonClickDepth)) {
		//maximum number of combinations reached.
		log("Too many attempts to click login button and still haven't seen FB traffic, probably failed to locate login button.");
		log("Site doesn't support FB login?\n", true);
		automatedTesting.finishedTesting(true);			//we consider this as non-failure tests.
		return;
	...
	if (loginClickAttempts >= 1){
		...
		for (i = 0; i < pressLoginButtonWorkers.length; i++){
			if (!pressLoginButtonWorkers[i].previousCandidates || pressLoginButtonWorkers[i].candidatesWithPreviousCriteria != pressLoginButtonWorkers[i].previousCandidates){
				same = false;
				break;
			}
		}
		if (same){
			console.log("Previous click is futile, fast forwarding...");
			if (conf.USENIX.experiments.testLoginButton){
				var recordString = "Failed:";
				recordString = recordLoginStatus(recordString);
				log(recordString, true);			
	...
	try {
		console.log("clicking on "+(indexToClickThisAttempt+1).toString()+"th ranked login element in the pool (for the " + (loginClickAttempts+1).toString() + "th click:");
...
exports.initPressLoginButton = function(worker){
	...
	pressLoginButtonWorker.port.on("reportCandidates", function(response){
		pressLoginButtonWorker.candidatesWithPreviousCriteria = response.candidatesWithPreviousCriteria;
		pressLoginButtonWorker.candidatesWithCurrentCriteria = response.candidatesWithCurrentCriteria;
