...
function sendPressLoginButton(response){
	...
	shouldClick = shouldClick || checkToken.shouldClickLoginButton() || checkCode.shouldClickLoginButton() || checkSR.shouldClickLoginButton();
	if (shouldClick) {
...
exports.initAutomateSSOWorker = function(worker){
	...
	automateSSOWorker.port.on("requestFBAccount",function(){
		automateSSOWorker.port.emit("requestFBAccount",{FBAccount:FBAccount, shouldAutomateSSO:(yetToInitCredentials || capturingPhase == 3 || capturingPhase == 9 || checkToken.shouldAutomateSSO() || checkCode.shouldAutomateSSO() || checkSR.shouldAutomateSSO())});
	});
