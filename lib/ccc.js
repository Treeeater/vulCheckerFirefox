...
function sendPressLoginButton(response){
	...
	if (shouldClick) {
		...
		try {	
			...
			console.log("waiting for page to load, 10sec .... ");
		...
		try {pressLoginButtonWorker.port.emit("pressedLoginButton",{"shouldClick":shouldClick, "tryFindInvisibleLoginButton":tryFindInvisibleLoginButton, "indexToClick": (loginClickAttempts == 1 ? indexToClick : indexToClick2), "loginClickAttempts":loginClickAttempts});} catch(ex){console.log("pressLoginButtonWorker hidden frame error 1");}
...
function checkLoginButtonRemoved(){
	...
	try{
		...
		if (checkLoginButtonRemovedAttempts < 2){
			console.log("pressloginworker hidden frame error - likely caused by host page still loading, will try again in 10 seconds.");
			setTimeout(checkLoginButtonRemoved, 10000);
		}
		else {
			console.log("pressloginworker hidden frame error - tried too many times, give up.");
		}
	}
}
...
function processLoaded(url){
	if (capturingPhase == 0 && checkAgainstFilter(url, capturingPhase))
	{
		...
		capturingPhase++;
		setTimeout( function(){try{testSuiteWorker.port.emit("action",{"action": "navigateTo", "site":siteToTest});} catch(ex){console.log("testSuiteWorker hidden frame error 5");}}, 2000);
		return;
	}
					...
					try {
						...
						if (newRedirectURI && newRedirectURI.indexOf('http')==0) {
							...
							if (newRedirectURI != redirectDomain){
								console.log("Redirect domain changed to: " + newRedirectURI);
...
exports.initPressLoginButton = function(worker){
	...
	pressLoginButtonWorker.port.on("checkTestingStatus", function(response){
		...
		var shouldClick = (capturingPhase == 2 || capturingPhase == 8);
		shouldClick = shouldClick || checkToken.shouldClickLoginButton();
		try {if (shouldClick) pressLoginButtonWorker.port.emit("checkTestingStatus",{"shouldClick":shouldClick, "account":accountsInfo});} catch(ex){console.log("pressLoginButtonWorker hidden frame error 3");}
	});
...
exports.initRegistrationWorker = function(worker){
	var prepareUserInfo = function (response){
		if (typeof accountsInfo != "undefined"){
			try {registrationWorker.port.emit("issueUserInfo",accountsInfo[FBAccount-1]);} catch (ex) {
...
exports.initIFramePressLoginButtonWorker = function(worker) {
	worker.port.on("checkTestingStatus", function(response){
		var shouldClick = (capturingPhase == 2 || capturingPhase == 8);
		shouldClick = shouldClick || checkToken.shouldClickLoginButton();
		try {if (shouldClick) worker.port.emit("checkTestingStatus",{shouldClick:shouldClick, indexToClick:indexToClick2, "account":accountsInfo, "loginClickAttempts":loginClickAttempts+1});} catch(ex){console.log("IFrame press login button worker hidden frame error");}			//Only gives indexToClick2 because we assume the first click is not from iframe, the second is.
	});
}
