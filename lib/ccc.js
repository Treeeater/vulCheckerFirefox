...
function clickLoginButton(){
	...
	if (loginClickAttempts >= LoginButtonClickDepth) {
		...
		if (indexToClickLoginButton >= Math.pow(LoginButtonCandidateSize,LoginButtonClickDepth)){
			...
			if (searchingForLoginButton){
				if (indexToClickLoginButton % LoginButtonCandidateSize == 0) {
					XPathHistory.push(curAttemptClicksXPathInfo[loginClickAttempts - 1]);			//record previously clicked xpath info
				}
				loginClickAttempts = 0;
				loginButtonClicked = false;
				loginButtonClickHistory = [];
				...
				log(tempStr);
				try {testSuiteWorker.port.emit("action",{"action": "navigateTo", "site":siteToTest});} catch(ex){log("testSuiteWorker hidden frame error 13");}
			}
		}
		closeAllOtherTabs();
		return;
	...
	if (indexToClickThisAttempt >= sortedCandidates.length) {
		...
		if (searchingForLoginButton){
			if (loginClickAttempts >= 1) {XPathHistory.push(curAttemptClicksXPathInfo[loginClickAttempts]);}			//record previously clicked xpath info
			fastForwardNextLoginButton();
		}
		return;
	}
	var sortedCandidatesXPath = sortedCandidates.map(function(ele,index,arr){return ele.XPath;}).sort().join("\n");		//sort alphabetically.
	if (XPathHistory.indexOf(sortedCandidatesXPath)!=-1){
		...
		if (searchingForLoginButton){
			log("We have seen this scenario before after the last click, fast forwarding...");
			if (conf.USENIX.experiments.testLoginButton){
				var recordString = "Failed:";
	...
	try {
		...
		if (loginClickAttempts >= 1) curAttemptClicksXPathInfo[loginClickAttempts] = sortedCandidatesXPath;
		shouldRemoveOldLoginWorkers = true;
