...
var testSuiteWorker;
var testSuiteWorkerFB;
var testSuiteWorkerApp;
...
function delayRefreshTestTab()
{
	...
	if (capturingPhase == 4 || capturingPhase == 10) {
		testSuiteWorkerApp.port.emit("action",{"action": "navigateTo", "site":siteToTest});
	}
}
...
function processLoaded(url){
	if (capturingPhase == 0 && checkAgainstFilter(url, capturingPhase))
	{
		...
		setTimeout( function(){capturingPhase++;testSuiteWorkerApp.port.emit("action",{"action": "navigateTo", "site":siteToTest});}, 2000);
		return;
	}
...
exports.initAutomateSSOWorker = function(worker){
	//if (capturingPhase != 3 || capturingPhase != 9) return;
...
exports.initTestSuiteWorker = function(worker){
	if (capturingPhase == 3 || capturingPhase == 9) { testSuiteWorkerFB = worker; testSuiteWorker = worker; }
	else { testSuiteWorkerApp = worker; testSuiteWorker = worker; temp++; console.log(temp);}
