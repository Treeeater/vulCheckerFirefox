...
function startOver(){
	...
	clearTimeout(checkLoginButtonRemovedTimer);
	checkToken.cleanup();
}
...
function testSuitePhase4(url){
	...
	capturingPhase++;
	if (!registrationNeeded) checkLoginButtonRemovedTimer = setTimeout(checkLoginButtonRemoved, 12000);
...
function testRegisterSuccess(){
	...
	checkLoginButtonRemovedTimer = setTimeout(checkLoginButtonRemoved, 10000);
}
