...
var debug = (!(conf.debug == false)) && true;
var writeFlag = (!(conf.writeFlag == false)) && true;
var automatedTestingFlag = (!(conf.automatedTestingFlag == false)) && true;
var detectionMode = CONST.dm.access_token_vul;
if (conf.detectionMode) detectionMode = conf.detectionMode | detectionMode;
...
var checkRedirectionAndAdjust = function()
{
	...
	try {
		...
		window.setTimeout(checkRedirectionAndAdjust,10000);
	}
}
...
var check_and_redo_credentials = function () {
	...
	{
		if ((detectionMode & CONST.dm.access_token_vul) != 0 && typeof accountsInfo[i].access_token == 'undefined') {FBAccount = i + 1; URL = token_url; type = "access_token"; break;}
...
function sendPressLoginButton(response){
	...
	shouldClick = shouldClick || checkToken.shouldClickLoginButton() || checkCode.shouldClickLoginButton();
	if (shouldClick) {
			...
			if (indexToClick >= 2 && indexToClick2 >= 2){			
				...
				if (tryFindInvisibleLoginButton){
					...
					if (!searchForSignUpForFB){
						...
						log("Signup button search doesn't help, test still fails.", true);
						automatedTesting.finishedTesting(false);
					}
				}
				else {
...
function processLoaded(url){
	...
	capturingPhase = checkCode.processLoaded(url);			//code vulnerability
}
	...
	pressLoginButtonWorker.port.on("sendLoginButtonInformation", function(response){
		...
		if (response.loginButtonXPath == "USER_INFO_EXISTS!" && response.loginButtonOuterHTML == "USER_INFO_EXISTS!") {
			...
			if (storageRecord[siteToTest].facebookDialogOAuthResponse) {
				...
				if (usedFBSDK){
					log("However, this doesn't matter because FBSDK is used.");
					log(siteToTest + " is not vulnerable to [2], because FBSDK is used (oracle not working).", true);
				}
				if (!usedFBSDK && typeof res != "undefined" && res.indexOf('code')==-1) {
					log("However, this doesn't matter because code is not seen in this traffic.");
					log(siteToTest + " is not vulnerable to [2], code not spotted (oracle not working).", true);
				}
...
exports.initAutomateSSOWorker = function(worker){
	...
	automateSSOWorker.port.on("requestFBAccount",function(){
		automateSSOWorker.port.emit("requestFBAccount",{FBAccount:FBAccount, shouldAutomateSSO:(yetToInitCredentials || capturingPhase == 3 || capturingPhase == 9 || checkToken.shouldAutomateSSO() || checkCode.shouldAutomateSSO())});
	});
