...
var stallChecker = function (){
	...
	stallCheckerTimer = window.setTimeout(stallChecker, 180000);
}
...
var old_token = "CAADxRthhGccBANIku4Be0ZCJbxK2qVNqVbXLltHHP0OZBIdvowHSy87MifZAEP6vgPtSWLzkZB2zkG6hpX7nJ0eUvV9LbKq5PvoiIIu9btNUiq3fNtcWoZCtVu5X27EQMw1kWS7qmP0kiNgNQc1XYHLs4AkNhGp4ZD";
var old_code = "";
...
function startOver(){
	...
	window.clearTimeout(sendPressLoginButtonTimer);
...
var startTest = function(site){
	...
	window.setTimeout(checkRedirectionAndAdjust,10000);	//check if phase is > 0, if not, indicates the website redirected itself. We make adjustments according to it.
}
...
function sendPressLoginButton(response){
	...
	if (shouldClick) {
		...
		try {	
			...
			sendPressLoginButtonTimer = window.setTimeout(sendPressLoginButton,10000);			//wait longer for the page load.
			return;
		}
		...
		sendPressLoginButtonTimer = window.setTimeout(sendPressLoginButton,10000);
	}
}
...
function testSuitePhase3(url){
	...
	if (bufferedResponses[url].body.substr(0,42)=='<script type="text/javascript">var message' && usedFBSDK)
	{
		...
		if (!registrationNeeded){
			delayRefreshTestTabTimer = window.setTimeout(delayRefreshTestTab,15000);			//after 10 seconds, refresh the homepage.
		}
		else{
			window.setTimeout(tryToRegisterInMainFrame, 15000);				//if the site needs register, after 10 seconds, try register the user.
		}
	}
	else if (!usedFBSDK)
	{
...
function testSuitePhase4(url){
	...
	else window.setTimeout(extractContent,12000);
}
...
function tryToRegisterInMainFrame(){
	...
	if (iframeRegistrationSubmitted) {
		...
		window.setTimeout(testRegisterSuccess, 1000);
		return;							
	}
	try {
    ...
    observe: function(aSubject, aTopic, aData)
    {
        if (aTopic == "http-on-examine-response")
        {
			...
			if (checkAgainstFilter(url, capturingPhase)){
				...
				//if (true){
				if ((url.startsWith("https://www.facebook.com/dialog/oauth") || url.startsWith("http://www.facebook.com/dialog/oauth")) && !FBSDKDetermined){
	...
	pressLoginButtonWorker.port.on("sendLoginButtonInformation", function(response){
		...
		if (response.loginButtonXPath == "USER_INFO_EXISTS!" && response.loginButtonOuterHTML == "USER_INFO_EXISTS!") {
			...
			if (storageRecord[siteToTest].facebookDialogOAuthResponse) {
				...
				if (typeof res != "undefined" && res.indexOf('access_token')==-1) {
					log("However, this doesn't matter because access_token is not seen in this traffic.");
					log(siteToTest + " is not vulnerable, access_token not spotted (But oracle is not working).", true);
					startOver();
...
exports.initAutomateSSOWorker = function(worker){
	if (worker.tab.i == undefined && old_token!="")
	{
		...
		else automatedTesting.startTestIfHaventStarted();
	}
...
exports.initRegistrationWorker = function(worker){
	...
	registrationWorker.port.on("registrationSubmitted",function(response){
		...
		window.setTimeout(testRegisterSuccess,10000);			//after 10 seconds, test if registration is successful.
	});
