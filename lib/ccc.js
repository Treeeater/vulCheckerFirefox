...
function sendPressLoginButton(response){
	...
	if (loginClickAttempts > 2) {log("Too many attempts (>2) to click login button and still haven't seen FB traffic, probably failed to locate login button."); return;}
	loginClickAttempts++;
	var shouldClick = (capturingPhase == 2 || capturingPhase == 8);
	shouldClick = shouldClick || checkToken.shouldClickLoginButton();
	if (shouldClick) {
		try {pressLoginButtonWorker.port.emit("pressedLoginButton",{"shouldClick":shouldClick});} catch(ex){console.log("pressLoginButtonWorker hidden frame error");}
