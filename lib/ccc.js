...
var stallChecker = function (){
	...
	if (arguments.length == 0 && previousPhase == capturingPhase && previousSiteToTest == siteToTest && !automatedTesting.allTestDone()) {
	...
	stallCheckerTimer = window.setTimeout(stallChecker, 300000);
}
...
var old_token = "CAADxRthhGccBANi9kZCzMZC9Aj5a6vlRPsyO0r3ZC8yY9PmWN0AkS79fzjixw2AonxJAy4cZC1E4FDpf9op0M4Phd9Is4ZCfaUMZChbTJu5Xlf2RlGeuVGlrW9QaUzIEVmYSbFI5MiQitiZCgOz3ZB8LJ7FHEbsMChkZD";			//deprecated as of July 17, will have access_token info for each account in account.js
var old_code = "";
		...
		if (loginClickAttempts >= 2) {
			...
			if (indexToClick >= 2 && indexToClick2 >= 2){			
				...
				if (tryFindInvisibleLoginButton){
					...
					log("Site doesn't support FB login?", true);
