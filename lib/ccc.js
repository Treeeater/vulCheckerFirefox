...
var debug = true;
var IdPDomains = ["https://www.facebook.com/dialog/oauth", "https://www.facebook.com/dialog/permissions.request"];
...
function testSuitePhase10(url){
	...
	capturingPhase++;
	saveToFile(fileNameSanitize(siteToTest), JSON.stringify(storageRecord));
}
...
function processBuffer(url)
{
	...
	if (capturingPhase == 8 && checkAgainstFilter(url, capturingPhase) && loginButtonClicked)
	{
		//revisit the page without cookies
		testSuitePhase8(url);
		return;
...
exports.initPressLoginButton = function(worker){
	...
	pressLoginButtonWorker.port.on("pressedLoginButton", function(response){
			...
			try {pressLoginButtonWorker.port.emit("pressedLoginButton",{"shouldClick":shouldClick});} catch(ex) { console.log("Caught Exception: hidden frame error");}
		}
	);
	pressLoginButtonWorker.port.on("checkTestingStatus", function(response){
			var shouldClick = (capturingPhase == 2 || capturingPhase == 8);
			shouldClick = shouldClick || checkToken.shouldClickLoginButton();
			pressLoginButtonWorker.port.emit("checkTestingStatus",{"shouldClick":shouldClick});
		}
	);
