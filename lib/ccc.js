...
function clickLoginButton(){
	...
	if (loginClickAttempts >= LoginButtonClickDepth) {
		...
		if (conf.USENIX.experiments.exhaustiveSearch){
...
function testSuitePhase2(url){
	...
	if (conf.USENIX.experiments.recordLoginButton){
...
function processBuffer(url)
{
	...
	if (capturingPhase == 2 && checkAgainstFilter(url, capturingPhase) && loginButtonClicked && sawDialogOAuth)
	{
		testSuitePhase2(url);
		sawDialogOAuth = false;
		if (!conf.USENIX.experiments.exhaustiveSearch) searchingForLoginButton = false;			//in login button experiments, we never stop searching for login button.
		return;
	}
