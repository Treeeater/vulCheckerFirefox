...
var LoginButtonClickDepth = 2;
var LoginButtonCandidateSize = 3;
var elementsToFill = [];
var buttonToClick = [];
...
var checkAgainstFilter = function(url, capturingPhase)
{
	...
	else if (!usedFBSDK && redirectDomain != "" && (capturingPhase == 3 || capturingPhase == 9))
	{
		...
		if (url.startsWith(redirectDomain) && credentialsInserted && (url.indexOf('access_token=')!=-1 || url.indexOf('code=')!=-1 || url.indexOf('signed_request=')!=-1)) {
			return true;
		}
	}
	return false;
...
function clickLoginButton(){
			...
			for (j = 0; j < loginButtonPool[i].result.length; j++){
				if (loginButtonPool[i].result[j].XPath == loginButtonClickHistory[loginClickAttempts].XPath || loginButtonPool[i].result[j].outerHTML == loginButtonClickHistory[loginClickAttempts].outerHTML){
					...
					try {
						...
						clickLoginButtonTimer = window.setTimeout(collectCandidatesAndClickNext,10000);			//click again after 10 secs.	
						break;
					}
	...
	assume(capturingPhase <= 5, loginButtonClickHistory.length.toString()+"&"+loginClickAttempts.toString());
	if (loginClickAttempts >= LoginButtonClickDepth) {
		indexToClickLoginButton++;
		if (indexToClickLoginButton >= Math.pow(LoginButtonCandidateSize,LoginButtonClickDepth)){
			...
			if (searchingForLoginButton){
				...
				sawDialogOAuth = false;
				capturingPhase = 1;
				var tempStr = "Attempting to click ";
				for (i = 0; i < LoginButtonClickDepth; i++)
				{
					var temp = (indexToClickLoginButton % LoginButtonCandidateSize)+1;
					if (i > 0) temp = Math.floor(indexToClickLoginButton/(Math.pow(LoginButtonCandidateSize,i))) + 1;
	...
	var indexToClickThisAttempt = (loginClickAttempts == 0) ? (indexToClickLoginButton % LoginButtonCandidateSize) : Math.floor(indexToClickLoginButton/(Math.pow(LoginButtonCandidateSize,loginClickAttempts)));
	...
	var breakFlag;
	var pressedEarlier;
	var sortedCandidates = new Array();
	while (true){
		maxScore = -999;
		maxIndex = -1;
		breakFlag = 0;
	...
	try {
		console.log("clicking on "+indexToClickThisAttempt.toString()+"th ranked element in the pool:");
		console.log(sortedCandidates[indexToClickThisAttempt].XPath);
		console.log(sortedCandidates[indexToClickThisAttempt].outerHTML);
	...
	catch(ex){log("pressLoginButtonWorker hidden frame error 1");}
	clickLoginButtonTimer = window.setTimeout(collectCandidatesAndClickNext,10000);			//check after 10 secs if a page nav has occured.
}
...
function checkLoginButtonExistenceAndReportToVulChecker(worker){
	...
	if (checkToken.shouldReturnLoginButtonInfoToVulChecker()){
		checkToken.checkLoginButtonRemoved(login_button_exists && !user_info_exists);
	}
	else if (checkSR.shouldReturnLoginButtonInfoToVulChecker()){
...
exports.initPressLoginButton = function(worker){
	...
	//listen to events
	if (clickLoginButtonTimer) window.clearTimeout(clickLoginButtonTimer);			//this happens when a new page loads. When this happens, we want to clear the old timer, and try to click the login button in this new page.
	pressLoginButtonWorker.port.on("reportCandidates", function(response){
		//main worker reported, we should attempt to click one after a certain time (waiting for iframes to report as well, but we have an upper limit).
		if (capturingPhase == 2 || capturingPhase == 8 || checkToken.shouldClickLoginButton() || checkSR.shouldClickLoginButton()){
			loginButtonPool.push({worker:pressLoginButtonWorker, result:response});
			window.setTimeout(clickLoginButton,1000);
		}
		...
		if (checkToken.shouldReturnLoginButtonInfoToVulChecker() || checkSR.shouldReturnLoginButtonInfoToVulChecker()){
			loginButtonPool.push({worker:pressLoginButtonWorker, result:response});
	...
	if (capturingPhase == 2 || capturingPhase == 8 || checkToken.shouldClickLoginButton() || checkSR.shouldClickLoginButton()) {
		window.setTimeout(function(response){
			...
			try {
				if (shouldClick) {
					pressLoginButtonWorker.port.emit("reportCandidates",{"account":accountsInfo, "searchForSignUpForFB":searchForSignUpForFB, "loginClickAttempts":loginClickAttempts});
				}
			} catch(ex){log("pressLoginButtonWorker hidden frame error 3");}
		}, 5000);
	}
	if (capturingPhase == 5 || capturingPhase == 11){
		if (!registrationNeeded) checkLoginButtonRemovedTimer = window.setTimeout(checkLoginButtonRemoved, 5000);			//5000 for iframe loading
		else extractContentTimer = window.setTimeout(extractContent,5000);
	}
	if (checkToken.shouldReturnLoginButtonInfoToVulChecker() || checkSR.shouldReturnLoginButtonInfoToVulChecker()){
		window.setTimeout(checkLoginButtonRemoved,5000);
	}
};
...
exports.initIFramePressLoginButtonWorker = function(worker) {
	...
	worker.port.on("reportCandidates", function(response){
		...
		loginButtonPool.push({worker:thisIframeWorker, result:response});
	});
	var shouldClick = (capturingPhase == 2 || capturingPhase == 8);
	shouldClick = shouldClick || checkToken.shouldClickLoginButton() || checkSR.shouldClickLoginButton();
	try {
		if (shouldClick) thisIframeWorker.port.emit("reportCandidates",{"account":accountsInfo, "searchForSignUpForFB":searchForSignUpForFB, "loginClickAttempts":loginClickAttempts});
	}
	catch(ex){};
