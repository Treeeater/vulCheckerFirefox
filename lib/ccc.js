...
var facebookWidgetButtonExist = false;		//used to indicate whether the social widget button still exists after login attempt.
...
function startOver(){
	...
	facebookWidgetButtonExist = false;
...
function testSuitePhase4(url){
	...
	facebookWidgetButtonExist = false;		//reset facebookWidgetButtonExist so that previous reported buttons are ignored.
...
exports.initPressLoginButton = function(worker){
	...
	pressLoginButtonWorker.port.on("sendLoginButtonInformation", function(response){
		...
		else if (loginButtonXPath == "FACEBOOK_PLUGIN!") {
			//user info doesn't exist, the login button xpath doesn't match. This could because the site used widget buttons, and their xpath is not actually recorded.  After login the site requires registration, and the login button is still there.
			if (facebookWidgetButtonExist) {
				log("login failed! After logging in the login button is still present!");
				loginFailure = true;
			}
		}
...
exports.initIFramePressLoginButtonWorker = function(worker) {
	worker.port.on("checkTestingStatus", function(response){
		...
		shouldClick = shouldClick || checkToken.shouldClickLoginButton() || checkSR.shouldClickLoginButton();
		try {
			worker.port.emit("checkTestingStatus",{shouldClick:shouldClick, indexToClick:indexToClick2, "account":accountsInfo, "loginClickAttempts":loginClickAttempts+1, "debug":debug, "searchForSignUpForFB":searchForSignUpForFB, "iframeClickedXPATH":iframeClickedXPATH, "iframeClickedOuterHTML":iframeClickedOuterHTML, "tryFindInvisibleLoginButton":tryFindInvisibleLoginButton});
			//Only gives indexToClick2 because we assume the only one click from iframe, so indexToClick doesn't differ from indexToClick2.'
		} 
		catch(ex){
			log("IFrame press login button worker hidden frame error");
		}			
	});
	...
	worker.port.on("reportLoginButtonFromIframe",function(response){
		facebookWidgetButtonExist = true;			//checkToken and checkSR.js doesn't need to be aware of this var.  This var is only used in phase 1-10 to help SSOScan complete the registration process, after that, it's not needed.
	});
