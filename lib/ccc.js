...
{
	var i = 0;
	if ((capturingPhase == 2 || capturingPhase == 8) && url.indexOf("http://www.facebook.com/dialog/return")==0) 
	{
		//special situation for websites using social plugin button.php, see mapquest.com as an example.
		if (redirectDomain != "http://static.ak.facebook.com/connect/xd_arbiter.php"){
...
function sendPressLoginButton(response){
	...
	if (shouldClick) {
		...
		try {	
			...
			sendPressLoginButtonTimer = window.setTimeout(sendPressLoginButton,6000);			//wait longer for the page load.
			return;
		}
		...
		sendPressLoginButtonTimer = window.setTimeout(sendPressLoginButton,6000);
	}
}
...
function testSuitePhase3(url){
	...
	if (bufferedResponses[url].body.substr(0,42)=='<script type="text/javascript">var message' && usedFBSDK)
	{
		...
		if (!registrationNeeded){
			delayRefreshTestTabTimer = window.setTimeout(delayRefreshTestTab,10000);			//after 10 seconds, refresh the homepage.
		}
		else{
...
function testSuitePhase9(url){
	...
	if (bufferedResponses[url].body.substr(0,42)=='<script type="text/javascript">var message' && usedFBSDK)
	{
		...
		capturingPhase++;
		if (!registrationNeeded){
			delayRefreshTestTabTimer = window.setTimeout(delayRefreshTestTab,10000);			//after 15 seconds, refresh the homepage.
		}
		else{
			window.setTimeout(tryToRegisterInMainFrame, 15000);				//if the site needs register, after 10 seconds, try register the user.
		}
