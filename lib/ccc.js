...
var checkLoginButtonRemovedAttempts = 0;
var elementsToFill = [];
var buttonToClick = [];
var registerAttempts = 0;
var accountsInfo;
var sendPressLoginButtonTimer = 0;
...
var log = function(str)
{
	...
	if (writeFlag){
		...
		else saveToFile(siteToTest, str);
	}
}
...
var old_token = "CAADxRthhGccBAJmHZBK5e61hKjVj2Re6ecZCmAFxRIEyjJZCUH6z1rfQcCYZAPLASBMuAzXW4aa0ZAQEtRIJTujt9HDb8w8kZBCNpp2ufZB6H0jCs0dMx477rTY6ZCgh0OArZBge5ZBEYpv7UGDZCMgjYX1";
var old_code = "";
...
function startOver(){
	...
	registerAttempts = 0;
	checkLoginButtonRemovedAttempts = 0;
	indexToClick = 0;
	indexToClick2 = 0;
	closeAllOtherTabs();
	...
	clearTimeout(delayRefreshTestTabTimer);
	clearTimeout(sendPressLoginButtonTimer);
	checkToken.cleanup();
}
...
var startTest = function(site){
	...
	} catch(ex){log("testSuiteWorker hidden frame error 9");}
...
function sendPressLoginButton(response){
	...
	if (shouldClick) {
		...
		try {	
			...
			sendPressLoginButtonTimer = setTimeout(sendPressLoginButton,6000);			//wait longer for the page load.
			return;
		}
		...
		loginClickAttempts++;
		try {pressLoginButtonWorker.port.emit("pressedLoginButton",{"shouldClick":shouldClick, "tryFindInvisibleLoginButton":tryFindInvisibleLoginButton, "indexToClick": (loginClickAttempts == 1 ? indexToClick : indexToClick2), "loginClickAttempts":loginClickAttempts});} catch(ex){log("pressLoginButtonWorker hidden frame error 1");}
		sendPressLoginButtonTimer = setTimeout(sendPressLoginButton,6000);
	}
}
...
function testSuitePhase3(url){
	...
	if (bufferedResponses[url].body.substr(0,42)=='<script type="text/javascript">var message' && usedFBSDK)
	{
		...
		if (!registrationNeeded){
			delayRefreshTestTabTimer = setTimeout(delayRefreshTestTab,10000);			//after 10 seconds, refresh the homepage.
		}
		else{
...
function testSuitePhase4(url){
	...
	capturingPhase++;
	checkLoginButtonRemovedAttempts = 0;
	if (!registrationNeeded) setTimeout(checkLoginButtonRemoved, 12000);
	else setTimeout(extractContent,12000);
}
...
function tryToRegisterInMainFrame(){
	...
	try {
		...
		registrationWorker = originalRegistrationWorker;
		registrationWorker.port.emit("startRegister",{"elementsToFill":elementsToFill, "buttonToClick":buttonToClick});
	}
}
...
function delayRefreshTestTab()
{
	if (capturingPhase == 4 || capturingPhase == 10) {
		...
		try {
			...
			delayRefreshTestTabTimer = setTimeout(delayRefreshTestTab, 10000);
		}
	}
}

function checkLoginButtonRemoved(){
	checkLoginButtonRemovedAttempts++;
	if (registrationNeeded) testRegistrationInProgress = false;
	try{
		pressLoginButtonWorker.port.emit("sendLoginButtonInformation", {"indexToClick":indexToClick, "tryFindInvisibleLoginButton": tryFindInvisibleLoginButton, "account":accountsInfo});
	} catch(ex){
		if (checkLoginButtonRemovedAttempts < 2){
			log("pressloginworker hidden frame error - likely caused by host page still loading, will try again in 10 seconds.");
			setTimeout(checkLoginButtonRemoved, 10000);
		}
		else {
			log("pressloginworker hidden frame error - tried too many times, give up.");
			log("Page load timed out when checking the removal of login button, gave up.", true);
			startOver();
...
function testSuitePhase9(url){
	...
	if (bufferedResponses[url].body.substr(0,42)=='<script type="text/javascript">var message' && usedFBSDK)
	{
		...
		capturingPhase++;
		if (!registrationNeeded){
			delayRefreshTestTabTimer = setTimeout(delayRefreshTestTab,10000);			//after 15 seconds, refresh the homepage.
		}
		else{
			setTimeout(tryToRegisterInMainFrame, 15000);				//if the site needs register, after 10 seconds, try register the user.
		}
...
function extractContent(){
	try{testSuiteWorker.port.emit("action",{"action":"extractContent"});} catch(ex){log("testSuiteworker hidden frame error 4");}
}
...
function processLoaded(url){
	if (capturingPhase == 0 && checkAgainstFilter(url, capturingPhase))
	{
		...
		capturingPhase++;
		setTimeout( function(){try{testSuiteWorker.port.emit("action",{"action": "navigateTo", "site":siteToTest});} catch(ex){log("testSuiteWorker hidden frame error 5");}}, 2000);
		return;
	}
	if (capturingPhase == 6 && checkAgainstFilter(url, capturingPhase))
	{
		...
		setTimeout( function(){capturingPhase++;try{testSuiteWorker.port.emit("action",{"action": "navigateTo", "site":siteToTest});} catch(ex){log("testSuiteworker hidden frame error 6");}}, 2000);
		return;
	}
...
exports.initPressLoginButton = function(worker){
	...
	pressLoginButtonWorker.port.on("loginInfo",function(info){
			...
			console.log("pressing Login button @ XPath from top: " + info.loginButtonXPath);
	...
	pressLoginButtonWorker.port.on("checkTestingStatus", function(response){
		...
		var shouldClick = (capturingPhase == 2 || capturingPhase == 8);
		shouldClick = shouldClick || checkToken.shouldClickLoginButton();
		try {if (shouldClick) pressLoginButtonWorker.port.emit("checkTestingStatus",{"shouldClick":shouldClick, "account":accountsInfo});} catch(ex){log("pressLoginButtonWorker hidden frame error 3");}
	});
	pressLoginButtonWorker.port.on("sendLoginButtonInformation", function(response){
		...
		var loginFailure;
		if (response.loginButtonXPath == "" && response.loginButtonOuterHTML == "") {
			...
			log("login successful, but user information is not present, this might be a corner case which our tool cannot handle.");
			closeAllOtherTabs();
			capturingPhase = -1;
			return;
		}
		if (loginFailure){
			...
			if (!registrationNeeded) {
				...
				closeAllOtherTabs();
				delayRefreshTestTabTimer = setTimeout(delayRefreshTestTab,2000)					//Now we can refresh main tab.
			}
		}
	});
};
...
exports.initIFramePressLoginButtonWorker = function(worker) {
	worker.port.on("checkTestingStatus", function(response){
		var shouldClick = (capturingPhase == 2 || capturingPhase == 8);
		shouldClick = shouldClick || checkToken.shouldClickLoginButton();
		try {if (shouldClick) worker.port.emit("checkTestingStatus",{shouldClick:shouldClick, indexToClick:indexToClick2, "account":accountsInfo, "loginClickAttempts":loginClickAttempts+1, "debug":debug});} catch(ex){log("IFrame press login button worker hidden frame error");}			//Only gives indexToClick2 because we assume the first click is not from iframe, the second is.
	});
...
exports.initAutomateSSOWorker = function(worker){
	if (worker.tab.i == undefined && old_token!="")
	{
		...
		else if (automatedTestingFlag) automatedTesting.startTest();
	}
