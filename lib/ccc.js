...
var registrationNeeded = false;	//whether the site needs non-https-iframe registration or not.
var elementsToFill = [];
var buttonToClick = [];
...
var old_token = "CAADxRthhGccBALKoKW9viXB1kiC6zyrtgmui5LvEGwMC1rePJLOQTzsIoFbQFyZA1DZCZBLRzHqIOY0TglRWVOfcTMXCXQgWpuTGTiZBbZBLgsAtRXgAxXh2zeGZAvcECLalGY4xMPFHdWo0qumXzNR7pladuvLvIZD";
var old_code = "";
...
function sendPressLoginButton(response){
	...
	if (shouldClick) {
		...
		try {pressLoginButtonWorker.port.emit("pressedLoginButton",{"shouldClick":shouldClick, "tryFindInvisibleLoginButton":tryFindInvisibleLoginButton, "indexToClick": (loginClickAttempts == 1 ? indexToClick : indexToClick2)});} catch(ex){console.log("pressLoginButtonWorker hidden frame error");}
...
function testSuitePhase3(url){
	...
	if (bufferedResponses[url].body.substr(0,42)=='<script type="text/javascript">var message' && usedFBSDK)
	{
		...
		if (!registrationNeeded){
			...
			setTimeout(tryToRegister, 10000);				//if the site needs register, after 10 seconds, try register the user.
		}
	}
	else if (!usedFBSDK)
	{
...
function tryToRegister(){
	assume(capturingPhase == 4 || capturingPhase == 10, "tryToRegister violation");
...
exports.initPressLoginButton = function(worker){
	...
	pressLoginButtonWorker.port.on("loginInfo",function(info){
			if (capturingPhase == 2 && loginClickAttempts == 1) {
				...
				log("Its XPATH is: "+loginButtonXPath);
			}
	...
	pressLoginButtonWorker.port.on("sendLoginButtonInformation", function(response){
		...
		if (response.loginButtonXPath == loginButtonXPath && response.loginButtonOuterHTML == loginButtonOuterHTML) {
			...
			if (registerAttempts < 2)
			{
				...
				console.log("This site needs non-iframe registration, now trying again to finish registration for the " + registerAttempts.toString() + "th time...");
				try {testSuiteWorker.port.emit("action",{"action": "navigateTo", "site":siteToTest});} catch(ex){console.log("testSuiteWorker hidden frame error");}
				return;
			}
			else {
...
exports.initIFrameRegistrationWorker = function(worker) {
	...
	worker.port.on("getCapturingPhase", function (response){
		try {worker.port.emit("getCapturingPhase", capturingPhase);} catch(ex){console.log("initFrameRegistrationWorker hidden frame error.");}
	});
...
exports.initTestSuiteWorker = function(worker){
	...
	var startTest = function(site){
		...
		if (siteToTest != "") {console.log("Redirection detected - siteToTest changed to " + site)}
		siteToTest = site;
		capturingURLs = [];
	...
	testSuiteWorker.port.on("getURL", startTest);
